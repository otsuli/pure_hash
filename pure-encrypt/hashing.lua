    ...
    local function process_chunk(chunk)
        ...
        for i = 1, 64 do
            if i <= 16 then--SHA-256 message schedule 
                ...
                local s0 = utils.bit_ror_right(w[i - 15], 7) ~ utils.bit_ror_right(w[i - 15], 18) ~ (w[i - 15] >> 3)  
                local s1 = utils.bit_ror_right(w[i - 2], 17) ~ utils.bit_ror_right(w[i - 2], 19) ~ (w[i - 2] >> 10)
        ...
        local S1 = utils.bit_ror_right(e, 6) ~ utils.bit_ror_right(e, 11) ~ utils.bit_ror_right(e, 25)
            local ch = (e & f) ~ ((~e) & g)
            local temp1 = (h + S1 + ch + k[i] + w[i]) & 0xFFFFFFFF
            local S0 = utils.bit_ror_right(a, 2) ~ utils.bit_ror_right(a, 13) ~ utils.bit_ror_right(a, 22)
